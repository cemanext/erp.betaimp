...
//          echo '<h1>$differenza_totale_pagato_parziale = '.$differenza_totale_pagato_parziale.'</h1>';
            ...
            */
            ...
            $sql_0004 = "SELECT 
               ...
               stato,
               IF(sezionale!='',IF(lista_fatture.stato LIKE 'In Attesa di Emissione' AND lista_fatture.tipo LIKE 'Fattura',CONCAT('<a class=\"btn btn-circle btn-icon-only green-jungle\" href=\"salva.php?tbl=lista_fatture&idFatturaEmettere=',id,'&codSezionale=',sezionale,'&fn=emettiFattura\" title=\"EMETTI\" alt=\"EMETTI\"><i class=\"fa fa-sign-out\"></i></a>'),IF(lista_fatture.stato LIKE 'In Attesa di Emissione' AND lista_fatture.tipo LIKE 'Nota di Credito',CONCAT('<a class=\"btn btn-circle btn-icon-only red-intense\" href=\"salva.php?tbl=lista_fatture&idFatturaEmettere=',id,'&codSezionale=',sezionale,'&fn=emettiNotaDiCredito\" title=\"EMETTI NOTA DI CREDITO\" alt=\"EMETTI NOTA DI CREDITO\"><i class=\"fa fa-sign-out\"></i></a>'),'')),'<b>Manca il Sezionale</b>') AS 'EMETTI'
               FROM lista_fatture WHERE id=".$id;
