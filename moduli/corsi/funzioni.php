...
function Stampa_HTML_Dettaglio_Corsi($tabella,$id){
    ...
    switch ($tabella) {
        ...
        case 'lista_corsi':
            ...
            $numRowBase = $dblink->num_rows("SELECT * FROM lista_corsi_configurazioni WHERE id_corso = '$id' AND titolo LIKE 'Base'");
            if($numRowBase <= 0){
                $dblink->query("INSERT INTO `lista_corsi_configurazioni` (`id`, `dataagg`, `scrittore`, `stato`, `id_corso`, `id_prodotto`,titolo, avanzamento, id_attestato, messaggio, firma, email_oggetto, email_mittente, email_messaggio) SELECT '', NOW(), '".addslashes($_SESSION['cognome_nome_utente'])."', 'Attivo', id, id_prodotto, 'Base', '".PERTENTUALE_ATTESTATO_BASE."', '".ID_ATTESTATO_BASE."', '".TESTO_CONFIGURAZIONE_ATTESTATO."', '".FIRMA_CONFIGURAZIONE_ATTESTATO."', 'INVIO ATTESTATO', '".EMAIL_DA_CONFIGURAZIONE_ATTESTATO."', '".EMAIL_TESTO_CONFIGURAZIONE_ATTESTATO."' FROM lista_corsi WHERE id='".$id."'");
            }
            $sql_0001 = "SELECT id, 
            CONCAT('<a class=\"btn btn-circle btn-icon-only blue btn-outline\" href=\"modifica.php?tbl=lista_corsi_configurazioni&id=',id,'\" title=\"MODIFICA\" alt=\"MODIFICA\"><i class=\"fa fa-edit\"></i></a>') AS 'fa fa-edit',
